...
Date: 2026-09-16. Bead `imd3`; owner coordination boundary `p2z9`. **Approval chronology:** the original design and offline implementation were approved; implementation then stopped on the system JSON parser blocker. The user subsequently explicitly approved the [parser revision](../plans/2026-09-16-synthetic-archive-parser-revision.md) and resumption of the original offline implementation. The runtime/diagnostic amendment below records that approval, not a passing implementation test result. Native builds, real archive inspection, credential access, provider operations, uploads and candidate wiring remain unauthorized. Owner acceptance has not been obtained.
...
The approved offline implementation uses installed Ruby standard library only: `/opt/homebrew/Cellar/ruby/4.0.7/bin/ruby --disable-gems`, JSON **2.18.0**, executable SHA-256 `0abc5dafa91bf31774888e70580d9b7d2739006beecf37795d3a2c3d383b974f`, subject to fresh containment/runtime/loaded-library identity checks. Its public function accepts **two in-memory JSON strings**, an expected fixture and observed evidence, and returns a bounded JSON-compatible result object. No CLI, filesystem/process/network/provider access, Fastlane loading, path dereference, archive extraction, signing tools, environment discovery or clock access. Merely requiring the module must have no such side effects.

Do not require `release_candidate.rb` or `release_contract.rb`: their broader dependency surfaces are unnecessary here. Standalone duplicate-key rejection must work before parse collapse without importing those modules. The pinned parser bypasses the tested custom Hash insertion override: do not depend on `object_class`/`[]=` interception. Explicit `allow_duplicate_key: false` is mandatory. This contract is not an adapter implementation and must never return `true`.
...
Each input is a UTF-8 JSON string of at most **65,536 bytes**. Reject invalid UTF-8, BOM, trailing non-whitespace, duplicate object keys at any nesting, excessive nesting, nonfinite numeric extensions and non-JSON values. Do not silently overwrite duplicate keys, coerce values, stringify arbitrary objects or call input-defined methods. Use the pinned JSON parser with explicit `allow_duplicate_key: false`, `create_additions: false`, `allow_nan: false`, and `max_nesting: 12`, not a post-parse duplicate check. Duplicate and other syntax errors both return `INPUT_SYNTAX`; typed `JSON::NestingError` is caught before its parser-error superclass and returns `INPUT_LIMIT`. Do not classify errors using message regex, echo exception text, introduce an alternate lexer, or fall back to another runtime/parser.
...
Input validation follows explicit stages: (1) check input string/UTF-8/BOM (`FIELD_SHAPE` for non-string input, otherwise `INPUT_ENCODING`); (2) check raw byte cap (`INPUT_LIMIT`); (3) parse with duplicate-key and nesting defenses, returning the first encountered parser-classified `INPUT_SYNTAX` or `INPUT_LIMIT` failure; (4) check remaining node/string/object/array safety caps (`INPUT_LIMIT`); (5) check schema/mode (`SCHEMA_MODE`); (6) check required fields, allowlisted keys, container and element scalar types (`FIELD_SHAPE`); (7) for the expected fixture only, check semantic cardinality/uniqueness and internal bindings (`EXPECTED_CONFLICT`). There is no duplicate-specific diagnostic priority: the first typed nesting failure maps to `INPUT_LIMIT`; other parser failures, including duplicates, map to `INPUT_SYNTAX`. Fixtures must pin that rule. This deliberately removes `DUPLICATE_KEY` from the output vocabulary while retaining duplicate rejection; it does not relax accepted syntax. Observed inventory cardinality/uniqueness is semantic, not a stage-6 shape failure. No raw parser exception, offending key or input excerpt is emitted.
