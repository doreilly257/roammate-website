...
5. Test shutdown closes/increments generation before completing the synchronous delivery shutdown barrier outside the lock. Registry closure prevents new ownership and rejects completions that have not passed their final generation check, but is not an instantaneous no-offer cutoff. An already generation-validated completion may offer or resume while shutdown is still in progress, before the delivery barrier completes; eligibility epochs remain checked at offer and resume. Add a deterministic intermediate-shutdown test for that overlap and prove **no new request may resume after shutdown returns**, including concurrent/repeated shutdown callers. Do not nest registry and delivery locks or imply already resumed traffic can be recalled. Run processor suite and retained snapshot/lifecycle suites. No SDK raw object may be retained by a delivery closure.
...
Root records scoped changed-file hashes, independent review results, production-source compile evidence and known runtime/release limits in the [scoped verification report](../verification/2026-09-14-production-telemetry-integration.md). Keep `sie` and broad released-journey acceptance blocked until their separate evidence exists. Close only the approved implementation Bead when its local acceptance is genuinely met; otherwise identify the exact remaining gate in Beads. Website landing steps belong to root; owner files remain local under this approval.

## September 14 execution evidence amendment

Units A–E now have passing local evidence and specification/code-quality approval. Combined root run `6278/a964b0` exited zero with 1,139 assertions (226 + 63 + 136 + 79 + 635), 318 SDK sources and two macro-denial receipts. It compiled normal nonfixture branches and the verbatim auth guest-mode property, not the full app. Fresh retained run `88098/a2967d` exited zero with the documented 91/64/2/73/3/45 cases, 314 SDK sources and two macro-denial receipts. Fresh integrated static review approved with no blockers. Six new production Swift files implement the proposed boundaries.

Preservation receipt `388687` found 16 prior worktree prerequisites unchanged, 19/20 owner snapshot files unchanged and all four owner telemetry boundary files unchanged. The differing owner `ImageCacheServiceTests.swift` was neither edited nor reverted by this slice. Auth run `93144/4ee66f` exited zero under the outer deny-network sandbox: 15 social matrices, unchanged email properties and all 10 social-completion sites. Worktree `git diff --check` passed in receipt `98b2fb`. Root reports all scoped local implementation gates complete. No owner merge, full app build, launch, additional sending or release follows from these results. See the report for exact scope; root owns the scoped implementation Bead update, not broader acceptance closure.
